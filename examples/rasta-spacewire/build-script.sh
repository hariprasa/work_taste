...
cd "$SKELS" && rm -f nodea.zip && zip nodea nodea/* && cd $OLDPWD

cd "$SKELS" && rm -f nodeb.zip && zip nodeb nodeb/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subC nodea:"$SKELS"/nodea.zip \
	--subC nodeb:"$SKELS"/nodeb.zip \
